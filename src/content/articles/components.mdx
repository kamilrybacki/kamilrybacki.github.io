...
Here, I've made a small distinction when it comes to how I perceive the functionality of the code that I want to present. Both of those subcategories
...
<CaptionedImage
  src="/assets/images/jupyter_real_time.png"
  alt="Jupyter Notebook real-time modification"
/>
...
JupyterLite project allows You to modify and extend Your own deployments of this environment if You want, for example, to include kernels for other languages. This also allows you to modify the source code (which will become useful in a second) and theme it according to Your needs. So I want to use the same method but with my version of JupyterLite, like so:
...
<CaptionedImage
  src="/assets/images/jupyterlite_plan.jpg"
  alt="RetroLab customization plan"
/>
...
Next, inside this `retro` directory, there was a `notebooks` one, that contained an HTML file that is used to display chosen notebooks.
...
Do You see that `import` in the last `script` tag 👀? That's our way in 🏃. It loads a `config-utils.js` file that is responsible for loading the whole RetroLab app. So, I've extended this to now load three separate scripts: `pre.js`, `main.js` and `post.js`. 
...
The `main.js` is simply the renamed `config-utils.js`. The `pre.js` and `post.js` are my scripts that are loaded before and after the `main.js` respectively. The `pre.js`, as of right now, is not used, but I've set it up for the future. The main hero here is the `post.js` script. It waits for the moment when the HTML elements of Jupyter Notebook are rendered and hydrated with content to morph them into the desired shape. To achieve this timed behavior, a `waitForElementToExist` method is used, that employs a `MutationObserver` to watch for the changes being made in the DOM. When the desired element is found, the observer is disconnected and the element is returned.
...
<CaptionedImage
  src="/assets/images/retrolab_help.png"
  alt="My custom JupyterLite help section"
/>
...
clicks on a button. This is done by using a `JupyterLiteEmbed` component, which is an
implementation of `<SomethingThatWrapsMyIFrame>` from the previous section.

The plan is simple: The user clicks a button, the component state is updated to `loading`, the spinner starts spinnin' 🌀,
...
a light version mangling of some dependencies in `package.json` to make it work with `jupyterlite`.
...
After building JupyterLite with this custom extension and deploying it to Vercel, I started working on
...
CodePen provides an awesome API that allows You to create pre-filled embeds programmatically, by setting appropriate `dataset` attributes of the container `iframe` element.
...
The on-demand loading is simply carried by the `__CPEmbed` function that loads the pen into an HTML Element specified by a DOM selector. The function is called
...
But in data science, the graphs are often cool and interesting, they draw You in with their content, so additional styling is only the cherry on top 🍒. For this goal, I've chosen the `rough-viz` JavaScript library and implemented a wrapper component (another one!) that adds stuff like a stylized title, caption and axis labels. Its main job is to apply the theme after `rough-viz` finishes initializing its graph element, because it is done via a constructor, that, when fired up, latches onto a selected `div` by its ID. There is one caveat during the application of the colors - the `rough-viz` graphs are normally created in "light mode", so colored data lines with **black axis lines**. But, if I first invert the colors of the lines, create the graph and then invert **the colors of the whole graph**, I get the desired effect - my theme colors set for the data series and white axis lines 🎨📈.
...
<ScribbleGraph
  ...
  height={200}
  title="Test"
  caption="Here is a scatter graph"
  options={{
    xLabel: 'X',
    yLabel: 'Y',
  }}
...
And if we insert another graph, it will be automatically numbered as "Figure 2".
...
So, that's it. I've presented the main components that I've created for this blog. I've also shown how I've used them to create more interactive and visually appealing content.
I hope that this presentation of more and more complicated concepts with gradually evolving "helping hands" in the form of my custom components planted this crazy idea in Your head 🪴 to use more engaging measures of showing off catchy ideas. 

Also, I guess there is some practical knowledge about e.g. how `iframe`s can communicate with their host "embedder" or how to chisel out Your cool modifications in the JupyterLite monolith via kind-of code injection or actual Jupyterlab extensions. With these gadgets, I tend to present my technological ramblings unbound by the limitations of spoken word.

<CaptionedImage
  src="/assets/images/office_magic.gif"
  alt="My blog content being rendered on Your screen"
  height="400"
/>

Prepare to be amazed, I guess 🤷‍♀️.
