...
Do You see that `import` in the last `script` tag 👀? That's our way in 🏃. Basically, it loads a `config-utils.js` file that is responsible for loading the whole RetroLab app. So, I've extended this to now load three separate scripts: `pre.js`, `main.js` and `post.js`. 
