...
<StyledSVG
  src='images/staff'
  alt='Crystalheart Pulse-Staff'
  class='invert grayscale w-3/4 mx-auto my-8 h-auto'
/>
...
<StyledSVG
  src='images/nesting'
  alt='docker whale in a nest'
  class='invert grayscale w-2/3 lg:w-1/3 mx-auto my-8 h-auto'
/>
...
<CaptionedImage
  ...
  alt="Whale communication channel established 🐳"
  height="700"
/>
...
<CaptionedImage
  ...
  alt="The ping has been ponged 🏓"
  height="700"
/>

### Snake-in-a-box 🐍📦 (but not that hypercube one)
...
the `connector.py` module has to offer? A janky 🍑 wrapper around the basic `redis.StricRedis` methods
...
4. Run `pytest` and (as previously) - hope for the best. 🤞
...
since it they are almost identical to previous ones - only this time the `python:3.10` image is pulled 🤷):

<CaptionedImage
  src="/assets/images/nested_container_pytest_pong.png"
  alt="The ping has been ponged again, programmatically 🐍🏓"
  height="700"
/>

### Matryoshka of problems 🪆

So we have successfully created a mock test environment for our integration tests.
It is self-contained and somewhat "invisible" (or dead 💀) to the outside architecture of the organization,
but we can see it is alive and kicking from the inside 😸. However, some things about it may (and in practice - often will) 
create some practical issues. 😿
...
simple case of a single container, it may be a bit of an overkill 🙈. But it will easily scale to a more complex
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (multiline)"
  annotations={`
...
Here, just to be 💯% sure, I explicitly set its name to `test-network`. It's not really necessary,
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (subnet)"
  annotations={`
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (networking)"
  annotations={`
...
<CaptionedImage
  ...
  alt="The composed pinger has received its service-oriented pong 🐋🏓"
  height="400"
/>
...
tool (if we're talking about a setup oriented towards CD) for further analysis. The possibilities are there❕
...
We're talking about REST API here. 4️⃣0️⃣4️⃣☠️

## Covert DevOps 🕵️📦

<StyledSVG
  src='images/ciops'
  alt='Top secret CI/CD operations'
  class='invert grayscale w-2/3 mx-auto my-8 h-auto'
/>

We will go wild on this one, but it actually references one of the cases I've encountered in my work.
The main goal here is to provide a middle-man application, **an agent** 🕵️, that requests metrics
...
#### No. 1 and 2: Together we start, divided we fail 🏳️
...
so all should be good. Well... 🤷‍♂️. What does it mean that our generator is healthy?
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (depends_on 2)"
  annotations={`
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (Prom HC)"
  annotations={`
...
the official documentation on the [`mongo` commands] (there are like a billion of them 😵).

Now, the *creme de la creme* 🤌 of this section - tests runner with our module installed
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (Prom yml mount)"
  annotations={`
...
automagically 🪄. The script is rather simple because it adds a user for the `metrics` database,
...
Then we plop (mount) that bad boi 🦹 into the `docker-entrypoint-initdb.d` directory and we're good to go.
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (js mount)"
  annotations={``}
/>
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="docker-compose.yml (envvars)"
  annotations={`
    ...
    69:Here we can omit the IP address - we 100% won't be using it 🤷
  `}
/>
...
<CodeSnippet
  ...
  annotations={`
    ...
    8:I have an undying love for dataclasses, sorry 🤷
...
"whatever, just give me something 🤷", we will use the following query: `{job=job_name}`. This
...
its plugin called `pytest-order` to strictly enforce the order of the tests 👮‍♀️. This is needed because
...
With this approach, we initialize the agent **ONCE** ☝️ and just pass it to the tests that need it.
...
<CodeSnippet
  ...
  annotations={`
    1:Mark as the second test 2️⃣
    2:We can just put the initialized adapter fixture as an argument to pass it here 😃
...
<CodeSnippet
  ...
  annotations={`
    ...
    10:The job name is set automagically by the create_mongo_document method 🪄
...
<img src="/assets/images/independent.png" alt="Independent" style="margin-top:2rem;" height="500" />

<StyledSVG
  src="images/independence"
  alt="An independent Docker Compose stack"
  class='invert grayscale w-full mx-auto my-8 h-auto'
  height="500"
/>

## Containerization of independence 📦🇺🇸🎆
...
- using Docker Compose to create a containerized environment for our tests inside GitLab CI 🦊
- how to make the configuration and networking a little more readable and adjustable via environment variables
- a fun tool to generate mock metrics (thanks Grafana Labs team! 🙏)
- how to setup Prometheus and MongoDB instances in Docker Compose (that's a bonus! 💲)
...
people if not such worlds on our beautiful planet Earth? 🌍
...
criticism, please contact me via social media - I suspect that this Containerized Integration™ thought
experiment can be further improved and I would love to hear Your ideas on how to do that. Peace ✌️

<img src="/assets/images/peace-out.gif" alt="Bye" style="height:300px;margin-top:2rem;" />
