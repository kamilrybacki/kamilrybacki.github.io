...
**Note**: Whenever a **multiline string** is to be passed as a prop, the string **must be enclosed in curly brackets and backticks** to avoid issues with the MDX parser. This also prevents problems with indentations and newlines. So, two birds with one stone.
