---
title: "Particles of Rust I: The Box"
date: "2024-01-11"
...
to speed up the calculations. 🚅 Everything checks out. ✔️
...
combine it with the **new** thing I'm learning, which is Rust 🦀.

(Also, it's a good excuse to write about the topic I'm passionate about 🤷)

### Extending the animal farm 🐫🐍🦀
...
of the supercomputer I was using that my cool Python tools were taking too much memory and
my computational job was promptly terminated. 😅 With C++, the problem was gone, because I could
...
<StyledSVG
  src='images/missingram'
  alt='Lost RAM'
  class='invert grayscale w-11/12 mx-auto my-2 h-42 lg:h-72'
/>
...
The atoms will speak in crab tongue today. 🦀
...
<StyledSVG
  src='images/loop'
  alt='Loop diagram'
  class='invert grayscale w-11/12 mx-auto -my-28 lg:my-4 h-38 lg:h-42'
/>
...
to people who not familiar with the language.
...
#### Particle No. 1 - Organization through isolation 🏝️
...
<CodeSnippet
  ...
  annotations={`
    ...
    2: Cute portmanteau of Rust and molecular dynamics. ⚛️🦀
...
#### Particle No. 3 - Wait, what's &? 🤔

<StyledSVG
  src='images/reference'
  alt='Pointing to data'
  class='invert grayscale w-11/12 mx-auto my-8 h-52 lg:h-96'
/>
...
be met with blank stares and awkward silence. 😅 They simply don't know what is being talked about,
...
<CodeSnippet
  ...
  annotations={`
    ...
    5: We try to print the value of the variable, but it IS DEAD 💀
  `}
/>
...
and it will still (hopefully) be understood.** Take it or leave it. 🤷

<StyledSVG
  src='images/joke'
  alt='Yikes'
  class='invert grayscale w-11/12 mx-auto lg:my-8 h-52 lg:h-60'
/>
...
it is one of the reasons why I decided to learn Rust. 👆

#### Particle No. 4 - Modeling the simulation 📐
...
First of all, no inheritance, subtypes, polymorphism, etc. - it's all gone. 🤯
...
#### Particle No. 5 - structure of time ⏰
...
<CodeSnippet
  ...
  annotations={`
    ...
    10: CAVEAT - match statement requires us to borrow the tested value ❗
...
<StyledSVG
  src='images/ballsandsticks'
  alt='Balls and sticks'
  class='invert grayscale w-11/12 mx-auto my-8 h-52 lg:h-72'
/>
...
They both will have the same fields i.e. they will only differ in name:
...
#### Particle No. 6 - I have inverted matrices You wouldn't believe 🤖
...
This is **set in stone** 🪦. No matter what user will give as an input, `a` will always be
...
Because of linear algebra. 🤷 The `nalgebra` crate basically implements a lot of useful
...
<img src="/assets/images/dontneedalgebra.png" alt="No algebra lib needed" style="margin-top:1rem;margin-bottom:1rem;" height="500" />
...
if something goes wrong over time. 😅
...
And everything is stack-allocated, so it's fast. 🚀
...
<CodeSnippet
  ...
  annotations={`
    35-40: We can use the "zeros" method to initialize the vectors with zeros! 😸
  `}
/>
...
#### Particle No. 7 - Down the crabby hole 🦀
...
2. For each entry, we check its type:
  ...
  * If it is a `Yaml::Integer` (an integer), we parse it as a `f64` (**after dereferencing the pointer** 👉) and push it to a new vector
...
in which workflows are presented in the source code is really nice and easy to follow. 😍
...
in the documentation of [`Iterator` trait from the standard library]. I highly recommend it. 😉

#### Implementing Atom 3: The Return of the `impl` 🧎‍♂️
...
<CodeSnippet
  ...
  annotations={`
    2: REFERENCE to YAML, so it doesn't wither away 💀
...
### Boxing the infinite ♾️ 👉 📦

<StyledSVG
  src='images/boxinginf'
  alt='The simulation box'
  class='invert grayscale w-11/12 mx-auto my-8 h-52 lg:h-72'
/>
...
#### Particle No. 8 - If it quacks like a struct, it's probably a struct 🦆
...
print esoteric information to the screen in electronic moonspeak. 🌛
...
<CaptionedImage
  ...
  alt="Rust compilation errors are actually helpful, they suggest what we messed up 😱"
/>
...
treat them as "magic" functions that obfuscate some more complicated code. 😅
...
during a specific moment in time and if they will reside inside our simulation box or not.
...
In practice, this procedure is rather simple to implement **if we have a proper linear algebra library** 😉.
...
<CodeSnippet
  ...
  `}
  language="rust"
  filename="calculate_mapping_matrix method"
  annotations={`
...
feature of the `nalgebra` library. 👍
...
<CodeSnippet
  ...
  `}
  language="rust"
  filename="calculate_box_vectors method"
  annotations={`
...
The only thing left is throwing in nice `Display` for our box and we're done here. 🎉
...
<CodeSnippet
  ...
  `}
  language="rust"
  filename="scale_cell_basis function"
  annotations={`
...
#### Particle No. 9 - Everything, everywhere, all at once 👁️
...
Imagine there are, like, thousands of atoms in our system. We want to modify their positions
...
ways to spawn separate tasks performed on multiple threads without touching the MPI documentation 😌.
...
optimization problems in the future, we will deal with them when the time comes (and it always comes 😅).
Later, we will use the `par_iter_mut` method to perform more complicated operations on our atoms,
but it is nice to introduce it here, so we can get used to it. 😉
...
<CodeSnippet
  ...
  `}
  language="rust"
  filename="generate_lattice function"
  annotations={`
    8-23: I guess I will shorten it in the future, but for now - it works 🤷
    53-56: Here we relabel the atoms, so they have unique IDs
    55: Enumerate returns a tuple with index AND element
  `}
/>

For the part of the code with triple-nested loop, I recommend checking out the definition of
[Bravais Lattice], which is used to generate the crystal lattice mathematically. It should
clarify why I am adding whole vectors to the position of each atom. The `continue` statement
is used to skip the first iteration of the loop, since we don't want to replicate the original
copies of each atom. After this operation, we finally end up with a 3D grid of atoms, that
occupy a repeatable shape in space and this pattern can be repeated indefinitely along the
dimensions of the simulation box. This is the **crystal lattice**, used to model
the behavior of atoms in a solid-state system.

What if we wanted to model something that is not a crystal? For example, a liquid or a gas?
We can use the same approach, but instead of replicating the atoms in a regular, ordered way,
we could generate them randomly within the simulation box. Or just manually specify their
positions and tell `rustomics` to skip their replication across the 3D space. For now,
I will test my software on simple, ordered systems but later I will expand on this topic
when adding features specific to simulating fluids (and that's a very distant future 😅).

Here is the final result of our implementation. It is a structure composed of a [face-centered
cubic lattice of aluminum atoms]. The simulation box consists of 10 repetitions along each dimension
and the unit cell vectors are defined in Angstroms. I have used a software called VMD to visualize
the structure, but there are many other options available, like Ovito or Avogadro (it may take a while to load)

<CaptionedImage
  src="/assets/images/vmd_fcc_al.gif"
  alt="Face-centered cubic lattice of aluminum atoms generated by rustomics"
/>

This system can be generated by passing the following options to the `system` section
of the simulation script YAML file:
...
<CodeSnippet
  ...
  `}
  language="yaml"
  filename="Configuration of our system"
  annotations={`
    4-6: The unit cell vectors are defined in Angstroms
    7: This unit cell has a 4 atom basis
    8-15: The atoms are defined in fractional coordinates
    16: The system is infinite in all directions
    17: We will come back to unit systems in the next article
    18: We want to replicate the unit cell in all directions, 10 times
  `}
/>

## Sealing the package 📦✅

This is a good place to stop, for now. We have our system constructed and can proceed to
implement the **main loop** of our simulation, controlled by the simulation engine to
perform subsequent steps of the simulation i.e. integration of the equations of motion **I.E**
actual molecular dynamics.

As You can see, we have a lot of code already, where we've touched upon a lot of aspects of
programming in Rust. The basic understanding of those concepts really opens up our minds to the
main advantages of Rust. It reinvents some of the tropes seen in other languages, but in a way
that is not that **intrusive**. I have encountered a lot of videos and articles that explain
how people are "fighting with the borrow checker" in an antagonistic way, even though they
recognize the benefits of the language and may even be experienced programmers. **BUT**,
I think it's the matter of how imperative is Your approach to a language. 

More freedom in forcing the language to do **exactly** what and where You want carries a lot of unforseen
responsibility and risk. I mean, look at C - it literally lets You carve out explicit
portions of memory and do whatever You want with it - **you don't even have to clean up after
Yourself** (you should ofc, but there are no mechanisms to force You to do so). Rust is
a language that is **very** strict about what You can and cannot do, but has that **sweet spot**
of performance and low-level control over the code. It is a language that is close
to the metal, but also *close to the programmer. And I think a portion of freedom is a small price
to pay for the benefits of the Cult of Crab. 🦀

Is the code shown 100% optimized and following the best Rust practices? I mean, let's be real.
My experience with Rust is not to be measured in years, sadly tbh because I can see I
**really vibe** with it. In the future, the refactoring of the code will be inevitable, but for now,
we stick to the MVP goal and the "done is better than perfect" approach.

## What's next?

In the next article, I will implement other components of the aforementioned `Simulation` class,
mainly ones that are responsible for the integration of the equations of motion - so we will
have more physics and maths there, with some juicy implementation of numerical methods along the way.

All of the code presented in this article can be found in the [`rustomics` repository], but be aware
that it is in a very early stage of development. If You have any suggestions or questions, feel free
to contact me via e-mail or social media. Also, If You want to contribute to the project, I will
be more than happy to review Your pull requests (please for the love of God implement MEAM potential
for me, thank You 🙏).

I hope that You enjoyed this article, I've tried to mix the technical aspects of programming in Rust
with a practical example of how to use it to implement a piece of semi-scientific software. I will try to keep
this balance in the future, so stay tuned for more articles. And who knows - maybe `rustomics` will become
a legit simulation engine in the future - that would be dope. For now, see you in the next one! 👋
...
[face-centered cubic lattice of aluminum atoms]: https://arxiv.org/ftp/arxiv/papers/2002/2002.01562.pdf
[`rustomics` repository]: https://github.com/kamilrybacki/rustomics
