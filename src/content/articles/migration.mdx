...
From now on, rendered entries should be available under the `[siteURL]/[subpath]/[postSlug]` route.
...
I guess there is no need to explain why this is convenient. Moreover, more computational intensive operations on arrays of data can be easily delegated to the server, making the client-side experience more smooth, eliminating annoying "Loading..." views, when You simply reduce an array of frontmatter data before serving it to the user.

This separation, of course, opens up the possibility of fetching data not only from local files or APIs, but also from a headless CMS of Your choice and Astro offers a series of integrations with the most popular ones.
